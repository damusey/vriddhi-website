...
export const MetricsShowcase: React.FC = () => {
    ...
    return (
        <section className="py-24 bg-brand-charcoal text-brand-offwhite relative overflow-hidden">
            {/* Background elements */}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12">
                    {content.metrics.map((metric, idx) => (
                        <AnimatedSection key={idx} className="text-center group !mb-0">
                            ...
                            >
                                <div className="text-4xl md:text-6xl font-bold font-sans text-brand-offwhite mb-4 tracking-tighter">
                                    {metric.prefix && <span className="text-brand-peach/80 text-3xl align-top mr-1">{metric.prefix}</span>}
                                    {metric.value}
